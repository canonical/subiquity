...
class VM:
    ...
    def reboot(self) -> None:
        ...
        """
        self._poweroff()
        _runcmd(["virsh", "start", self.domain], stdout=DEVNULL)
    ...
    def _install(self, **kwargs) -> None:
        ...
        try:
            ...
            try:
                _runcmd(["virsh", "destroy", self.domain], check=False)

            finally:
                undefine = ["virsh", "undefine", self.domain, "--remove-all-storage"]
                _runcmd(undefine, stdout=DEVNULL)
    ...
    def _get_ip(self) -> Optional[str]:
        ...
        sp = _runcmd(cmd, stdout=PIPE, check=False)
        if sp.returncode != 0:
            return None
    ...
    def _wait_for_domstate(self, expected):
        def check_domstate() -> bool:
            sp = _runcmd(["virsh", "domstate", self.domain], stdout=PIPE)
