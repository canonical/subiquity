...
log = logging.getLogger("subiquity.ui.views.storage.delete")
    ...
    def __init__(self, parent, obj):
        ...
        if fs is not None:
            ...
            if obj.type == "lvm_volgroup":
                line = ngettext(
                    "It contains 1 logical volume", "It contains {n} logical volumes", n
                )
            else:
                line = ngettext(
                    "It contains 1 partition", "It contains {n} partitions", n
                )
            ...
            line = ngettext("It contains 1 volume.", "It contains {n} volumes.", n)
            lines.append(Text(line.format(n=n)))
            lines.append(Text(""))
        else:
...
class ConfirmReformatStretchy(Stretchy):
    def __init__(self, parent, obj):
        ...
        fs = obj.fs()
        if fs is not None:
            title = _("Remove filesystem from {device}").format(device=labels.desc(obj))
            lines = [
                _(
