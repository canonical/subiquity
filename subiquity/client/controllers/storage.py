...
from subiquity.ui.views import GuidedDiskSelectionView, StorageView
...
class StorageController(SubiquityTuiController, StorageManipulator):
    ...
    async def run_answers(self):
        ...
        if self.answers["tpm-default"]:
            self.app.answers["storage-confirmed"] = True
        ...
        if self.answers["guided"]:
            ...
            self.app.answers["storage-confirmed"] = True
            while not isinstance(self.ui.body, StorageView):
                await asyncio.sleep(0.1)
            self.finish()
    ...
    async def _answers_action(self, action):
        from subiquity.ui.views.storage.delete import (
    ...
    def finish(self):
        log.debug("StorageController.finish next_screen")
