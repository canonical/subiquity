...
class FilesystemManipulator:
    ...
    async def delete_raid(self, raid):
        if raid is None:
            return
    ...
    async def delete_volgroup(self, vg):
    ...
    async def create_logical_volume(self, vg, spec: LogicalVolumeSpec):
    ...
    async def delete_logical_volume(self, lv):
        await self.clear(lv)
    ...
    async def volgroup_handler(self, existing, spec: VolGroupSpec):
        if existing is not None:
            key = spec.get("passphrase")
