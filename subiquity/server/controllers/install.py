...
class InstallController(SubiquityController):
    ...
    async def curtin_install(self, *, context, source):
        ...
        if storage_controller.reset_partition_only:
        ...
        elif storage_controller.use_snapd_install_api():
            await run_curtin_step(
                name="partitioning",
                stages=["partitioning"],
                step_config=self.filesystem_config(
        ...
        rp = storage_controller.model.reset_partition
        if rp is not None:
    ...
    async def postinstall(self, *, context):
        ...
        if self.supports_apt():
            ...
            if (
                ...
                and not storage_controller.use_snapd_install_api()
            ):
                with context.child(
