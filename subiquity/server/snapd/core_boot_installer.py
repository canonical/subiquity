...
@attrs.define
class CoreBootInstallPlan:
    """Bridge state shared between the storage controller and the installer.
...
class CoreBootInstaller:
    ...
    def __init__(self, app, plan: CoreBootInstallPlan) -> None:
        self.app = app
    ...
    async def setup_encryption(
        ...
        if self.plan.volumes_auth is not None:
            kwargs["volumes_auth"] = self.plan.volumes_auth
    ...
    async def snapd_target_preseed(self, target: pathlib.Path) -> None:
        async with AsyncExitStack() as es:
            ...
            await snapdapi.post_and_wait(
                self.app.snapdapi,
                self.app.snapdapi.v2.systems[self.plan.info.label].POST,
                snapdtypes.SystemActionRequest(
                    action=snapdtypes.SystemAction.INSTALL,
    ...
    async def finish_install(self, context, kernel_components: list[str]) -> None:
        ...
        label = self.plan.info.label
        kernels = self.plan.info.system.model.snaps_of_type(
            snapdtypes.ModelSnapType.KERNEL
        )
        if len(kernels) == 1:
            optional_snaps = []
            if (optionals := self.plan.info.system.available_optional) is not None:
