...
log = logging.getLogger("subiquity.models.storage")
...
class RecoveryKeyHandler:
    ...
    def generate(self):
        ...
        self._key = StorageModel.generate_recovery_key()
...
def fsobj(typ):
    def wrapper(c):
        ...
        c.__annotations__["_m"] = "StorageModel"
...
# in the StorageModel or StorageController classes.
...
class _Device(_Formattable, ABC):
    ...
    def available(self):
        ...
        from subiquity.common.storage.gaps import largest_gap_size
    ...
    def _available(self):
        ...
        if self._mount is None:
            if self.preserve:
                ...
                return StorageModel.is_mounted_filesystem(self.fstype)
        else:
            return False
...
class ActionRenderMode(enum.Enum):
    # The default for StorageModel.render() is to render actions
...
class StorageModel:
    ...
    def _render_actions(self, mode: ActionRenderMode = ActionRenderMode.DEFAULT):
        ...
        def emit(obj):
            if isinstance(obj, Raid):
                log.debug(
                    "StorageModel: estimated size of %s %s is %s",
                    obj.raidlevel,
                    obj.name,
                    obj.size,
                )
    ...
    def all_devices(self):
        ...
        for a in self._actions:
            if a.type == "disk":
                ...
                compounds.append(a)
        compounds.reverse()
        from subiquity.common.storage import labels
    ...
    def remove_partition(self, part, *, allow_renumbering=True, allow_moving=True):
        ...
        from subiquity.common.storage.gaps import (
