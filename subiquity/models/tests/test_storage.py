...
def make_disk(storage_model=None, **kw):
    if storage_model is None:
        storage_model = make_model()
    if "serial" not in kw:
        kw["serial"] = "serial%s" % len(storage_model._actions)
    if "path" not in kw:
        kw["path"] = "/dev/thing%s" % len(storage_model._actions)
    if "ptable" not in kw:
        kw["ptable"] = "gpt"
    size = kw.pop("size", 100 * (2**30))
    storage_model._actions.append(
        Disk(m=storage_model, info=FakeStorageInfo(size=size), **kw)
    )
    disk = storage_model._actions[-1]
    return disk
...
class TestOnRemoteStorage(SubiTestCase):
    def test_disk__on_local_storage(self):
        ...
        d = make_disk(
            storage_model=m, name="nvme1n1", nvme_controller=ctrler, serial="pcie1"
        )
        self.assertFalse(d.on_remote_storage())
    ...
    def test_disk__on_remote_storage(self):
        ...
        d = make_disk(
            storage_model=m, name="nvme0n1", nvme_controller=ctrler, serial="tcp0"
        )
    ...
    def test_is_rootfs_on_remote_storage_with_zfs(self):
        ...
        d = make_disk(storage_model=m, name="nvme0n1", nvme_controller=ctrler)
        pool = make_zpool(model=m, device=d, pool="pool0")
        make_zfs(
            model=m,
            pool=pool,
