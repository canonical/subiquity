...
class NetworkConfigForm(Form):
    ...
    nameservers = StringField(
        _("Name servers:"), help=_("IP addresses, comma separated")
    )
    searchdomains = StringField(
        _("Search domains:"), help=_("Domains, comma separated")
    )
    ...
    def clean_subnet(self, subnet):
        if "/" not in subnet:
            if self.ip_version == 4:
                ...
                example = "xx:xx:..:xx/yy"
            raise ValueError(
                _("should be in CIDR form ({example})").format(example=example)
            )
...
class EditNetworkStretchy(Stretchy):
    def __init__(self, parent, dev_info, ip_version):
        ...
        self.method_form.method.caption = _("IPv{v} Method: ").format(v=ip_version)
        manual_initial = {}
    ...
    def done(self, sender):
        if self.method_form.method.value == "manual":
            ...
            log.debug(
                "EditNetworkStretchy %s manual config=%s", self.ip_version, config
            )
            self.parent.controller.set_static_config(
                self.dev_info.name, self.ip_version, config
            )
        elif self.method_form.method.value == "dhcp":
            self.parent.controller.enable_dhcp(self.dev_info.name, self.ip_version)
            log.debug("EditNetworkStretchy %s, dhcp", self.ip_version)
        else:
            self.parent.controller.disable_network(self.dev_info.name, self.ip_version)
...
class VlanForm(Form):
    ...
    def clean_vlan(self, value):
        ...
        if vlanid is None or vlanid < 1 or vlanid > 4095:
            raise ValueError(_("VLAN ID must be between 1 and 4095"))
        return vlanid
...
class BondForm(Form):
    ...
    def _select_level(self, sender, new_value):
        ...
        self.lacp_rate.enabled = new_value in BondParameters.supports_lacp_rate

    def validate_name(self):
        name = self.name.value
        if name in self.all_netdev_names:
            return _('There is already a network device named "{netdev}"').format(
                netdev=name
            )
        if len(name) == 0:
...
class BondStretchy(Stretchy):
    def __init__(self, parent, existing_bond_info, candidate_names):
        ...
        connect_signal(self.form, "cancel", self.cancel)
        super().__init__(
            title, [Pile(self.form.as_rows()), Text(""), self.form.buttons], 0, 0
        )
